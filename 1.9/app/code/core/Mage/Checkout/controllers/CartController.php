...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
	...
	if ($this->getRequest()->getParam("remove")) {
		...
		Mage::log("items foo: " . print_r($cartItems), null, "rebatebus.log");
		...
		foreach ($this->_getCart()->getQuote()->getAllItems() as $item) {
			Mage::log('comparing sku: ' . $item->getSku(), null, 'rebatebus.log');
			if ($item->getSku() == 	$productId) {
				Mage::log('found sku: ' . $item->getSku(), null, 'rebatebus.log');
				$model = Mage::getModel('sales/quote_item_rebate');
				...
				$model->save();
				Mage::log('saved rebate ' . print_r($model->getData(), true), null, 'rebatebus.log');
				$this->_getSession()->addSuccess(
					'Rebate was applied'
				);
				$this->_goBack();
