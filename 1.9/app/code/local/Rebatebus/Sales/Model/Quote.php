...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
     ...
     * @return Mage_Sales_Model_Quote
     */
    public function removeAllRebates()
    {
        foreach ($this->getRebatesCollection() as $itemId => $item) {
            if (is_null($item->getId())) {
                $this->getRebatesCollection()->removeItemByKey($itemId);
            } else {
                $item->isDeleted(true);
            }
        }
    ...
    {
        return Mage::getModel('sales/quote_item_rebate')->getCollection();
    }
    ...
    {
        $items = array();
        foreach ($this->getRebatesCollection() as $item) {
            if (!$item->isDeleted()) {
                $items[] =  $item;
            }
        }
